...
| P17 | Preserve final-display semantics | Readable completion is DOM text, not first paint. Append observations previously checked glossary and project paths sequentially and could inherit a background page's next animation-frame wake-up. Cold/warm navigation still has sequential observation semantics. | Partially fixed 2026-08-08: append readable, glossary, and project-path completion now receive independent mutation-time browser-clock marks, while Playwright waits only validate correctness. Keep the DOM-not-paint and cold/warm sequential caveats; extend independent marks to navigation only when those subphase times become decision metrics. |
