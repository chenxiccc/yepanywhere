...
| P02 | Separate focused latency from bulk-fanout stress | The `focused-append` scale point now uses one project, one session, one cache-disabled page, and one appended turn under its own capacity/driver/scenario history key. Three Node 20 repetitions measured 4.4–5.0 ms selected-session server response and 51–54 ms final browser display, versus the deliberately contended fleet result. | Fixed 2026-08-08: CI runs the browser scale point on a fresh runner. Ratchet the focused server response at 50 ms, browser final display at 200 ms, and the two dominant browser child phases at 100 ms each; retain fleet append as the fan-out stress contract. |
...
| P09 | Bound global file-activity fanout and rescan work | Every activity subscriber still receives every changed file and filters client-side. | Partially fixed 2026-08-08: missing-filename and periodic rescans use asynchronous directory reads plus 64-file stat batches, coalesce overlap into one trailing pass, preserve exact-event races, and stop on lifecycle change. Focused targets in one directory share one native watch, with final-unsubscribe and late-resolution teardown tests. Server-side activity filtering remains and requires an explicit compatibility design. |
...
| P11 | Recover from incremental refresh failures | `packages/client/src/hooks/useSessionMessages.ts` silently discarded relay, parse, anchor, and server errors from the incremental `afterMessageId` path, leaving the open view stale. | Fixed 2026-08-08: a failed cursored refresh makes one uncursored request with the same compact-tail bounds through the per-session coordinator and atomically replaces stale state. A second failure stops without a timer or internal retry loop. Development and explicit remote-log diagnostics are limited to one report per route per 30 seconds and include the suppressed count. |
| P12 | Watch full-normalization and anchor-search scaling | Claude append parsing is incremental, but normalization rebuilds over the full raw array after append and normalized anchor search is linear. Recent appended p95 was 0.1–0.2 ms for normalization and 0.6–3.1 ms for route slicing/anchor search. | Dispositioned 2026-08-08: both owner phases remain separately visible and are ratcheted at 2 ms normalization plus 5 ms fleet/10 ms large-session routing. Investigate incremental normalization or indexed anchors only if a larger scale crosses those tripwires. |
| P13 | Watch client projection cache misses on append | Catch-up merge creates a new message-array identity, so transcript projection recompiles the loaded window. Recent appended preprocessing p95 was 0.3–0.8 ms. | Dispositioned 2026-08-08: browser preprocessing remains a separate child phase and is ratcheted at 5 ms across both cache budgets and scenarios. Optimize only if a larger loaded window crosses that tripwire. |
| P14 | Isolate external-summary batch contention | The realistic fixture now activates the eligible Claude watcher. At large scale it queues eight distinct summaries in one batch after about 300 ms; the batch takes 45–62 ms. The former 967–1,030 ms browser result followed the next wake-up of `requestAnimationFrame` polling in throttled headless pages, not the batch duration. | Dispositioned 2026-08-08: maintenance and suite results expose bounded queue/counter/timing events, and append final-display marks now come from an in-page `MutationObserver` armed before writes. Corrected watcher-on p95 is 417/435 ms uncached/cached versus 466/473 ms with the watcher disabled, so the 300 ms schedule remains unchanged. Reopen scheduling only on mutation-time or user-visible evidence; immediate ownership activity remains separate from batched complete summaries. |
| P15 | Measure useful cold readiness | The `built-client` driver now excludes a recorded production build, then uses separate fresh production servers for direct startup-to-transcript and cold built-page legs. Three Node 20 fleet repetitions measured 1.18–1.25 s server useful readiness and 776–822 ms client readable text; large-session repetitions measured 1.24–1.26 s and 1.16–1.19 s. | Fixed 2026-08-08: ratchet the direct selected-session needle at 2.5 s fleet/3 s large and built-client readable text at 1.5 s fleet/2.5 s large. Independent mutation marks validate glossary/path completion without Playwright wake bias. `perf-sweep` also caught a real Claude model probe escaping the old mock boundary. Explicit provider overrides now own current-source discovery; routine browser drivers disable discovery for cross-revision safety, and every suite server uses mock session launch. |
