...
Status: SPECULATIVE — proposal only. No experiments have run; every
result table is a TBD scaffold and every hypothesis is unverified.
Companion topic: [`topics/goal-judge-fork-vs-side-session.md`](../topics/goal-judge-fork-vs-side-session.md)
(mechanism inventory, experiment queue, product contract).
