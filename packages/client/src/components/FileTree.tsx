...
export const FileTree = memo(function FileTree({
  ...
  return (
    <div className={`file-tree ${className ?? ""}`}>
      {rootLoading ? (
        <div className="file-tree-loading">{t("gitStatusFileTreeLoading")}</div>
      ) : rootError ? (
        <div className="file-tree-error">
          ...
          <button
            ...
            onClick={loadRoot}
          >
            {t("gitStatusFileTreeRetry")}
          </button>
        </div>
      ) : filteredRootNodes.length === 0 ? (
        <div className="file-tree-empty">{t("gitStatusFileTreeEmpty")}</div>
      ) : (
        ...
        <div className="file-tree-connector">
          {error ? (
            ...
            <div
              ...
              style={{ paddingLeft: `${(depth + 1) * 12 + 4}px` }}
            >
              {t("gitStatusFileTreeLoadingShort")}
            </div>
          ) : children.length === 0 ? (
            <div
