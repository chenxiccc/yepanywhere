...
export function formatRelativeTime(timestamp: string, t: Translate): string {
  ...
  if (diffMins < 1) return t("sourceManagerRelativeNow");
  if (diffMins < 60) return t("sourceManagerRelativeMinutes", { count: diffMins });
  if (diffHours < 24) return t("sourceManagerRelativeHours", { count: diffHours });
  if (diffDays <= 30) return t("sourceManagerRelativeDays", { count: diffDays });
  if (diffMonths < 12)
    return t("sourceManagerRelativeMonths", { count: diffMonths });
  return t("sourceManagerRelativeYears", { count: diffYears });
}
