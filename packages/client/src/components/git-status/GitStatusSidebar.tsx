    ...
    <aside className="git-desktop-sidebar">
      <div className="git-desktop-card git-sidebar-primary-card">
        <div
          ...
          role="tablist"
          aria-label={t("sourceManagerSummary")}
        >
          <button
            type="button"
            role="tab"
            ...
            <span className="git-view-tab-label">
              ...
              {activeView === "files" ? (
                <span
                  ...
                  title={t("sourceManagerRefresh")}
                  aria-label={t("sourceManagerRefresh")}
                >
                  <RefreshIcon size={12} />
                </span>
              ) : null}
        ...
        {activeView === "files" ? (
          <div className="git-files-card">
            <div className="git-file-filter git-file-filter-standalone">
              <FileTreeSearch
                ...
                placeholder={t("sourceManagerFileSearchPlaceholder")}
                clearLabel={t("sourceManagerFileTreeClearSearch")}
              />
            </div>
            <FileTree
              projectId={projectId}
              ...
              <div className="git-desktop-file-groups">
                  ...
                  <div className="git-filter-bar">
                    <div className="git-filter-field">
                      ...
                      <input
                        ...
                        placeholder={t("sourceManagerFileFilterPlaceholder")}
                        className="git-filter-input"
                        aria-label={t("sourceManagerFileFilterPlaceholder")}
                      />
                      {fileFilter.length > 0 ? (
                        <button
                          type="button"
                ...
                {visibleFiles.length === 0 ? (
                  <div className="git-branch-merge-empty">
                    {t("sourceManagerFileFilterEmpty")}
                  </div>
                ) : null}
                {visibleFiles.length > 0 ? (
                  <GitFileSection
                    title={t("sourceManagerFilesChanged", {
          ...
          <div className="git-history-sidebar-card">
            {historyLoading ? (
              <div className="git-status-empty">
                {t("sourceManagerHistoryLoading")}
              </div>
            ) : historyError ? (
              <div className="git-diff-error">{historyError}</div>
            ) : historyCommits.length === 0 ? (
              <div className="git-status-empty">
                {t("sourceManagerHistoryEmpty")}
              </div>
            ) : (
                            ...
                            <button
                              ...
                              title={
                                copiedHash === commit.hash
                                  ? t("sourceManagerHistoryHashCopied")
                                  : t("sourceManagerHistoryCopyHash")
                              }
                              aria-label={
                                copiedHash === commit.hash
                                  ? t("sourceManagerHistoryHashCopied")
      ...
      {activeView === "changes" ? (
          <div className="git-desktop-card git-commit-card">
            <textarea
              ...
              rows={3}
              placeholder={t("sourceManagerCommitPlaceholder")}
            />
            <Button
              variant="primary"
