...
> Proposal: launch-time tooling for supervised sessions — shipped YA
> helper scripts on the session's PATH, a client-capability fragment
> telling the agent how to use them, and new-session options that shape
> the instruction environment — addressed throughout by canonical YA
> session ids.
...
feature — the vanilla instruction-scope option — is tracked in
[`gaps/vanilla-new-session-option.md`](../gaps/vanilla-new-session-option.md).
...
## Instruction-scope options ("vanilla" sessions)
...
configuration — a *vanilla* session in exactly the `vanilla-defaults.md`
sense of provider-default behavior. The per-provider mechanics differ:
...
- **Claude**: the Agent SDK's `settingSources` option already controls
  ...
  `packages/server/src/sdk/providers/claude.ts`. Vanilla drops
...
- **Codex**: no per-file switch exists; the mechanism is a redirected
  ...
  threading are specified in `gaps/vanilla-new-session-option.md`.
...
per session (`session-sandbox.ts`); a sandboxed vanilla session composes
